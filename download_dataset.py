...
def filter_and_download(split, ann_dir, max_images):
    ...
    # union of image ids across all target categories (getImgIds with a
    # multi-item catIds list is an AND filter, not OR, so union manually)
    img_ids = set()
    for cat_id in cat_ids:
        img_ids.update(coco.getImgIds(catIds=[cat_id]))
    img_ids = sorted(img_ids)[:max_images]
    print(f"{split}: {len(img_ids)} images matched (capped at {max_images})")
